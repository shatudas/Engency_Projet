         ...
         <form method="POST" action="{{ route('services.update',$editdata->id) }}" id="myForm" enctype="multipart/form-data">
          ...
          <div class="form-row">
           ...
           <div class="form-group col-md-6">
            ...
            <input type="text" name="title" class="form-control" value="{{ $editdata->title }}" placeholder="Enter  Service Title">
            <font style="color:red">{{ ($errors->has('title'))?($errors->first('title')):''  }}</font>
           </div>

           <div class="form-group col-md-12">
            <label for="heading">Heading <span style="color:red;">*</span></label>
            <input type="text" name="heading" class="form-control" value="{{ $editdata->heading }}" placeholder="Enter Service Heading">
            <font style="color:red">{{ ($errors->has('heading'))?($errors->first('heading')):'' }}</font>
           </div>

           <div class="form-group col-md-6">
            <label for="short_detalis">Short Detalis</label>
            <textarea type="text" name="short_detalis" class="form-control" placeholder="Enter Short Detalis" rows="5">{{ $editdata->short_detalis }}</textarea>
           </div>

           <div class="form-group col-md-6">
            <label for="description">Description</label>
            <textarea type="text" name="description" class="form-control" placeholder="Enter Service Description" rows="5">{{ $editdata->description }}</textarea>
           </div>
           ...
           <div class="form-group col-md-12">
            <img id="showImage" src="{{ !empty($editdata->image)?url('upload/Service_image/'.$editdata->image):url('upload/No-image.jpg') }}"  style=" height:120px; width:300px; border:1px solid #ccc;">
           </div>

           @php
            $datas = json_decode($editdata->services_point);
           @endphp
         
           @if( $datas != Null)
           @foreach($datas as $edit)

            <div class="delete_whole_extra_add" id="delete_whole_extra_add" style="width:100%;">
             <div class="add_item">
              <div class="form-row">

               <div class="form-group col-md-12">
                <input type="text" name="services_point[]" value="{{ $edit }}" class="form-control" placeholder="Enter Amount..">
               </div>
 ...
 $(function () {
  $('#myForm').validate({
   rules: {
    ...
    heading: {
        required: "Please Category Heading",
      },
    status: {
